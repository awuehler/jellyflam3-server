...
- Phase 5 synopsis (parked): two deployments — **A** JellyFlam3 Furnace (Pi 5) and **B** LLM Agent Platform (Arduino Ventuno Q). Homelab BOM + INT4 storage math; Llama 3.1 8B / Qwen2.5 7B / Mistral 7B (INT4); **one hot Instruct**; **pixels → text → Instruct JSON** with a small GPU VLM; **Jellyfin VoD as camera** for **single-sheep** loops ([phase5/02](docs/phase5/02_LLM_INTEGRATION.md#vod-as-camera)). [docs/phase5/](docs/phase5/00_OVERVIEW.md).
