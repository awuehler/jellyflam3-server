...
- Phase 5 synopsis (parked): two deployments — **A** JellyFlam3 Furnace (Pi 5) and **B** LLM Agent Platform (Arduino Ventuno Q). Homelab BOM + INT4 storage math; Llama 3.1 8B / Qwen2.5 7B / Mistral 7B (INT4) mapped to furnace contracts; **one hot graph** with stop → config → start session switch; Hexagon+Adreno hybrid LLM investigation ([phase5/03](docs/phase5/03_AI_PLATFORM_GAPS.md#g18--hexagon--adreno-during-one-llm-runtime-investigation)). [docs/phase5/](docs/phase5/00_OVERVIEW.md).
