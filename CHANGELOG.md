...
- Phase 4 tuples: one MP4 (loop A + watermarked edge A→B + loop B) under `by-generation/tuple/`; idle-cron mode; Roku shuffle includes `pedigree` and `tuple` (channel 1.0.28). Default edge mark is `Electric-Sheep-Icon-7A8B99.png` (`watermark.style: image`); look/feel documented in [phase4/03](docs/phase4/03_EDGES_AND_WATERMARK.md#look-and-feel-what-the-viewer-sees).
