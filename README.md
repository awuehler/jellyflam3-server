...
Homelab operators and enthusiasts who already run (or will run) **Jellyfin on a Raspberry Pi 5**, want a slow, quality-first generative flock — not a plug-and-play smart-TV app — and will either **sideload** the Roku clients or install VoD from the Channel Store once Roku review completes (`v0.3.2`). First sheep can take **hours**; a sizeable herd takes **months**. JellyFlam3 is **not** the official Electric Sheep client and is **not affiliated with** Spotworks LLC ([NOTICE](NOTICE)). See [User guide & runbook](docs/USER_GUIDE_AND_RUNBOOK.md) (Layer 1) for viewers; [fridge card](docs/FRIDGE_CARD.md) to print; [Pi from scratch](docs/phase2/09_PI_FROM_SCRATCH.md) for install.
...
- **Phase 4** — [docs/phase4/](docs/phase4/) · [00_OVERVIEW.md](docs/phase4/00_OVERVIEW.md)
- **Phase 5** — [docs/phase5/](docs/phase5/) · [00_OVERVIEW.md](docs/phase5/00_OVERVIEW.md)
