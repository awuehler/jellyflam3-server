...
- **Viewer feedback loop (Phase 4 — parked):** Roku VoD transient like/love/vote overlay (playback continues) → tallies on the catalog **sidecar** (sole metadata SoT) → share cron for Tailscale/Syncthing + weighted daily idle breed. Key `viewer_feedback` reserved in phase1/07; overlay/sink parked. Guide: [phase4/08_VIEWER_FEEDBACK_LOOP.md](phase4/08_VIEWER_FEEDBACK_LOOP.md).
...
1. Parked products: peer share-path, mesh introduce, **standalone edge files + loop/stills watermark**, **Roku publish**, **viewer feedback** overlay/sink, pasture filename/alias toggle — [phase4/00_OVERVIEW.md](phase4/00_OVERVIEW.md). Tuples + edge-stage watermark and 09 RNG aliases are **shipped**.
