...
- **Phase 3 (guides 01–10 complete — Owner OK 2026-08-23):** Roku stills screensaver (**01**); Kodi ES-dogma screensaver (**02** loops-only; loop→edge→loop as Phase 4 **tuples**); Sheep Shears (**03**); shared sheep security (**05**); git pedigree sheep (**06**); Hammer (**07**); Jellyfin ID dump + furnace client zip presets (**08**); **sheep refactor** (**09** — pathways A/P/B/C/D); acceptance (**10**). Git tag **`v0.3.0` at public launch**. DeepDream/social aspirational. Guides: [`docs/phase3/`](phase3/00_OVERVIEW.md). Post-launch **Phase 4** (**tuples + edge watermark shipped**; standalone edges cancelled; remaining: peer share-path, **Roku VoD + screensaver publish**, **viewer feedback loop**, pasture alias toggle): [`docs/phase4/`](phase4/00_OVERVIEW.md) (synopsis). **Phase 5** (parked): two deployments — Pi **furnace** vs Ventuno **LLM Agent Platform** (not overlapping) — [`docs/phase5/`](phase5/00_OVERVIEW.md).
...
- **Viewer feedback loop (Phase 4):** Roku VoD **1.0.32** transient like/love/vote overlay (playback continues) → tallies on the catalog **sidecar** (sole metadata SoT). Share cron + weighted daily idle breed parked. `POST /v1/sheep-votes` on display-sink. Guide: [phase4/08_VIEWER_FEEDBACK_LOOP.md](phase4/08_VIEWER_FEEDBACK_LOOP.md).
...
1. Parked products: **Roku Channel Store**, pasture filename/alias toggle — [phase4/00_OVERVIEW.md](phase4/00_OVERVIEW.md). **Cancelled:** standalone edge files + loop/stills watermark; screensaver voting; auto-promote (gated `promote --apply`). Tuples + edge-stage watermark, 09 RNG aliases, **08 overlay + share cron + breed weights**, **04 private-channel path**, **06 rotate**, and **02 mesh A/B/C** are **shipped**.
