...
- **Concurrent clients / link capacity (Phase 4 — estimator shipped 2026-09-03, Owner OK 2026-09-09):** `python3 -m pipeline.link_capacity` prints integer `N_max` from usable hop × headroom ÷ session bps. WiFi-uplinked Pi is the tight case. Guide: [phase4/07_CONCURRENT_CLIENTS.md](phase4/07_CONCURRENT_CLIENTS.md).
...
1. Parked products: peer **auto-promote**, **standalone edge files + loop/stills watermark**, **Roku Channel Store**, pasture filename/alias toggle — [phase4/00_OVERVIEW.md](phase4/00_OVERVIEW.md). Tuples + edge-stage watermark, 09 RNG aliases, **08 overlay + share cron + breed weights**, **04 private-channel path**, **06 rotate**, and **02 mesh A/B/C** are **shipped**.
