...
1. Peer share-path, mesh introduce scripting, **edges + watermark**, **Roku VoD + screensaver publish**, **library disk rotate**, **viewer feedback loop** (vote → share + breed bias), and **sheep naming** (auto aliases) parked under [phase4/00_OVERVIEW.md](phase4/00_OVERVIEW.md). End-user guide **05 baseline** (tasks / four examples / triage) is complete (Owner OK 2026-09-03); remaining 05 expansion waits on 08/09. Concurrent-client **07 estimator** shipped 2026-09-03 (Owner OK pending).
