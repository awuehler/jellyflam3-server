...
- **Phase 3 (guides 01–10 complete — Owner OK 2026-08-23):** Roku stills screensaver (**01**); Kodi ES-dogma screensaver (**02** loops-only; loop→edge→loop as Phase 4 **tuples**); Sheep Shears (**03**); shared sheep security (**05**); git pedigree sheep (**06**); Hammer (**07**); Jellyfin ID dump + furnace client zip presets (**08**); **sheep refactor** (**09** — pathways A/P/B/C/D); acceptance (**10**). Git tag **`v0.3.0` at public launch**. DeepDream/social aspirational. Guides: [`docs/phase3/`](phase3/00_OVERVIEW.md). Post-launch **Phase 4** (**tuples + edge watermark shipped**; standalone edges cancelled; remaining: Roku Channel Store): [`docs/phase4/`](phase4/00_OVERVIEW.md) (synopsis). **Phase 5** (parked): two deployments — Pi **furnace** vs Ventuno **LLM Agent Platform** (not overlapping) — [`docs/phase5/`](phase5/00_OVERVIEW.md).
...
- **Roku publish (Phase 4):** private-channel path so VoD + screensaver coexist (one sideload slot). Channel Store / brand assets parked. Guide: [phase4/04_ROKU_PUBLISH.md](phase4/04_ROKU_PUBLISH.md).
...
- **Sheep naming (Phase 4 — RNG shipped 2026-09-09):** auto-generated memorable aliases (`adjective_surname`) on the catalog sidecar; human override; `python3 -m pipeline.sheep_naming`. Roku/Kodi filename vs alias display stays parked. **LLM-from-poster → Phase 5** pixels → caption → Instruct JSON ([phase5/02_LLM_INTEGRATION.md](phase5/02_LLM_INTEGRATION.md#vision-pipeline)). Guide: [phase4/09_SHEEP_NAMING.md](phase4/09_SHEEP_NAMING.md).
...
1. Parked products: **Roku Channel Store** — [phase4/00_OVERVIEW.md](phase4/00_OVERVIEW.md). Optional Jellyfin OriginalTitle / SortName-as-alias held back. **Cancelled:** standalone edge files + loop/stills watermark; screensaver voting; auto-promote (gated `promote --apply`). Tuples + edge-stage watermark, 09 RNG aliases + **VoD titleMode** + **Kodi/Roku SS captions**, **08 overlay + share cron + breed weights**, **04 private-channel path**, **06 rotate**, and **02 mesh A/B/C** are **shipped**.
