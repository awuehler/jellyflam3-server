...
- **Phase 3 (guides 01–10 complete — Owner OK 2026-08-23):** Roku stills screensaver (**01**); Kodi ES-dogma screensaver (**02** loops-only; loop→edge→loop as Phase 4 **tuples**); Sheep Shears (**03**); shared sheep security (**05**); git pedigree sheep (**06**); Hammer (**07**); Jellyfin ID dump + furnace client zip presets (**08**); **sheep refactor** (**09** — pathways A/P/B/C/D); acceptance (**10**). Git tag **`v0.3.0` at public launch**. DeepDream/social aspirational. Guides: [`docs/phase3/`](phase3/00_OVERVIEW.md). Post-launch **Phase 4** (**tuples + edge watermark shipped**; standalone edges cancelled; remaining: Roku Channel Store; Kodi/SS alias captions): [`docs/phase4/`](phase4/00_OVERVIEW.md) (synopsis). **Phase 5** (parked): two deployments — Pi **furnace** vs Ventuno **LLM Agent Platform** (not overlapping) — [`docs/phase5/`](phase5/00_OVERVIEW.md).
...
- **Sheep library disk rotate (Phase 4 — 2026-09-13):** `library_disk check` + Shears rotate (`rotate --apply`, `cron_library_rotate.sh`); worker refuse on sheep BAD. Guide: [phase4/06_LIBRARY_DISK_ROTATE.md](phase4/06_LIBRARY_DISK_ROTATE.md).
...
1. Parked products: **Roku Channel Store**, Kodi/SS alias captions — [phase4/00_OVERVIEW.md](phase4/00_OVERVIEW.md). **Cancelled:** standalone edge files + loop/stills watermark; screensaver voting; auto-promote (gated `promote --apply`). Tuples + edge-stage watermark, 09 RNG aliases + **VoD titleMode**, **08 overlay + share cron + breed weights**, **04 private-channel path**, **06 rotate**, and **02 mesh A/B/C** are **shipped**.
