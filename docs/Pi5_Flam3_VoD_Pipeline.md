...
- **Concurrent clients / link capacity (Phase 4 — estimator shipped 2026-09-03):** `python3 -m pipeline.link_capacity` prints integer `N_max` from usable hop × headroom ÷ session bps. WiFi-uplinked Pi is the tight case. Guide: [phase4/07_CONCURRENT_CLIENTS.md](phase4/07_CONCURRENT_CLIENTS.md).
...
- **Sheep naming (Phase 4 — parked):** auto-generated memorable aliases (`adjective_surname`) on the catalog sidecar; human override; optional LLM-from-poster; Roku/Kodi filename vs alias display toggle. Keys `alias` / `alias_source` reserved in phase1/07; RNG parked. Guide: [phase4/09_SHEEP_NAMING.md](phase4/09_SHEEP_NAMING.md).
