...
- **Phase 3 (guides 01–10 complete — Owner OK 2026-08-23):** Roku stills screensaver (**01**); Kodi ES-dogma screensaver (**02** loops-only; loop→edge→loop as Phase 4 **tuples**); Sheep Shears (**03**); shared sheep security (**05**); git pedigree sheep (**06**); Hammer (**07**); Jellyfin ID dump + furnace client zip presets (**08**); **sheep refactor** (**09** — pathways A/P/B/C/D); acceptance (**10**). Git tag **`v0.3.0` at public launch**. DeepDream/social aspirational. Guides: [`docs/phase3/`](phase3/00_OVERVIEW.md). Post-launch **Phase 4 implemented** (`v0.3.2` — tuples, votes, naming, rotate, mesh introduce, VoD Channel Store submitted pending review; Dreams Store leftover): [`docs/phase4/`](phase4/00_OVERVIEW.md). **Phase 5** (parked): two deployments — Pi **furnace** vs Ventuno **LLM Agent Platform** (not overlapping) — [`docs/phase5/`](phase5/00_OVERVIEW.md).
...
3. **Shared settings** — VoD Settings, Screensaver Settings **1.0.10**, or a furnace-built zip writes `baseUrl`, `apiKey`, `userId`, `libraryId`, `commercialMode` to registry section `JellyFlam3`. Private-channel VoD does not share that section with sideload SS. SS Settings does **not** write `shuffleFlock`.
...
4. **While busy:** do not **start** new `flam3-animate` / encode jobs. The worker checks the gate at stage boundaries (claim, sequence, animate, encode). A live `flam3-animate` is **not** paused when Playing starts (`freeze_worker: false`). Drain is pause-before-next-inbox, not mid-frame. Checkpoint/resume inside `flam3-animate` and SIGSTOP-as-pause are **cancelled** (Phase 4, 2026-09-19).
...
- **Posters (Phase 2):** mid-loop frame via ffmpeg → `stills/{stem}/{stem}-poster.jpg` **and** Jellyfin Images API (base64 POST; `stills/.ignore` hides the JPEG from the library scan) with retry; **auto** ingest (standalone skip; 2+ live mesh furnaces create); backfill existing flock. Screensaver stills (non-tuple) extract and upload as Jellyfin Backdrops on the same path (same encoding).
...
- **Pedigree breeding (Phase 2):** flam3-genome mutate / **cross (= blend)** / interpolate; single-flame parents only; lineage sidecar. Daily idle cron: [`scripts/cron_breed_idle.sh`](../scripts/cron_breed_idle.sh) (one child when inbox empty). **LLM-assisted pedigree → Phase 5** ([phase5/02_LLM_INTEGRATION.md](phase5/02_LLM_INTEGRATION.md)).
...
- **Edges + watermark (Phase 4 — tuples shipped 2026-09-05):** one catalog MP4 (loop A + edge A→B + loop B) with edge-stage watermark. **Private mixed** furnaces may overlay the Cesari PNG; **commercial-safe / public** furnaces never burn that mascot (ES attribution text). An **operator PNG** in `watermark.image` overlays on **both**. Standalone `type: edge` files and watermark on loops/stills **cancelled** 2026-09-13. Guide: [phase4/03_EDGES_AND_WATERMARK.md](phase4/03_EDGES_AND_WATERMARK.md). Policy: [docs/media/watermark/README.md](media/watermark/README.md).
- **Roku publish (Phase 4 — implemented `v0.3.2`):** private-channel path so VoD + screensaver coexist (one sideload slot). VoD Channel Store **submitted** 2026-09-23 (pending review; unset 1.0.51 package). Dreams screensaver Store leftover. Guide: [phase4/04_ROKU_PUBLISH.md](phase4/04_ROKU_PUBLISH.md).
...
- **Sheep library disk rotate (Phase 4 — closed 2026-09-19):** `library_disk check` + Shears rotate (`rotate --apply`); optional `cron_library_rotate.sh` ([Activate daily rotate](phase4/06_LIBRARY_DISK_ROTATE.md#activate-daily-rotate)); worker refuse on sheep BAD.
- **Concurrent clients / link capacity (Phase 4 — estimator shipped 2026-09-03, Owner OK 2026-09-09):** `python3 -m pipeline.link_capacity` prints integer `N_max`. Lab hop is WiFi STA (`eth0 DOWN`); not a Jellyfin cap. Guide: [phase4/07_CONCURRENT_CLIENTS.md](phase4/07_CONCURRENT_CLIENTS.md).
- **Viewer feedback loop (Phase 4):** Roku VoD **1.0.32** transient like/love/vote overlay (playback continues) → tallies on the catalog **sidecar** (sole metadata SoT). Share cron + weighted daily idle breed. Operator `python3 -m pipeline.sheep_votes top` ranks live-catalog votes; `sweep --confirm SWEEP` zeros them (does not unshare `peers/share-out`). `POST /v1/sheep-votes` on display-sink. Guide: [phase4/08_VIEWER_FEEDBACK_LOOP.md](phase4/08_VIEWER_FEEDBACK_LOOP.md). Kodi SS votes: [phase5/05_KODI_SCREENSAVER_VOTES.md](phase5/05_KODI_SCREENSAVER_VOTES.md).
- **Sheep naming (Phase 4 — RNG + titles shipped):** auto-generated memorable aliases (`adjective_surname`) on the catalog sidecar; human override; `python3 -m pipeline.sheep_naming`. VoD / Kodi / Roku SS `titleMode`. Optional Jellyfin OriginalTitle / SortName-as-alias parked. **LLM-from-poster → Phase 5** pixels → caption → Instruct JSON ([phase5/02_LLM_INTEGRATION.md](phase5/02_LLM_INTEGRATION.md#vision-pipeline)). Guide: [phase4/09_SHEEP_NAMING.md](phase4/09_SHEEP_NAMING.md).
- **Furnace vs LLM Agent Platform (Phase 5 — parked):** **A** Raspberry Pi 5 remains the only JellyFlam3 furnace. **B** Arduino Ventuno Q is a separate local LLM/VLM host that advises A over LAN (naming, breed briefs). Not interchangeable; do not colocate. Guides: [phase5/00_OVERVIEW.md](phase5/00_OVERVIEW.md).
...
**F. Phase boundaries after Phase 1 complete:** Phase 2 = flock UX (posters), **HLS client streaming** (Jellyfin → Roku/VLC/etc., remux preferred), Roku polish + display probe, Syncthing-over-Tailscale peering (host-service Opt In/Out; `*.flam3` only; eventual pedigree-only share), **sheep tax** (XML/vocab scan & repair), pedigree mutate/cross, dynamic duration (hard max 120 s), Pi-from-scratch HW profiles. Phase 3 = Roku stills screensaver, **separate Kodi ES-dogma screensaver**, Sheep Shears CRUD, **sheep refactor** (sub-standard quality repair), **shared sheep security**, **git pedigree sheep** (smoke/examples; replace legacy samples). DeepDream/social stay aspirational. **Phase 4 implemented (`v0.3.2`)** = tuples + edge watermark (standalone edges **cancelled**), 05 user-guide, 06 disk check + rotate, 07 `N_max` estimator, 08 VoD vote overlay (Roku screensaver voting **cancelled**), 09 RNG aliases + titles, sidecar keys, VoD Channel Store submitted (pending review; Dreams Store leftover). **Phase 5** = LLM Agent Platform on Ventuno Q beside an existing Pi furnace (parked; not a 16-class port). Archive seeder + OkLCh tint + Gold Sheep Lite are **baseline shipped**.
...
| **Out** | Metadata rules table; catalog sidecar schema (shipped fields + Phase 4 reserved keys `type`, `watermark`, `viewer_feedback`, `alias`) |
...
| | |
|---|---|
| **Boundary** | Tuples (loop A + edge + loop B as one MP4) + edge-stage watermark — **shipped**; standalone edge files + loop/stills watermark **cancelled** |
| **In** | Two single-flame parents; `flam3-genome sequence` |
| **Tasks** | Tuple encode + edge-stage watermark (shipped); standalone edges / loop watermark **cancelled** |
| **Out** | Tuple MP4s under `by-generation/tuple/`; Cesari PNG (private) vs ES attribution (public) vs operator PNG (both) |
...
### Phase 4 — implemented (`v0.3.2`, 2026-09-23)

1. VoD Channel Store **submitted** (pending Roku review) — [phase4/04_ROKU_PUBLISH.md](phase4/04_ROKU_PUBLISH.md). **Leftover:** JellyFlam3 Dreams Store listing. Optional Jellyfin OriginalTitle / SortName-as-alias held back. **Cancelled:** standalone edge files + loop/stills watermark; Roku screensaver voting; auto-promote (gated `promote --apply`). Tuples + edge-stage watermark, 09 RNG aliases + VoD/SS titles, 08 overlay + share cron + breed weights, 04 private-channel path, 06 rotate, and 02 mesh A/B/C are **shipped**.
2. Pre-open slices (2026-09-03) closed into this tag: end-user guide **05 baseline**; library-disk **06**; concurrent-client **07 estimator**; sidecar keys in [phase1/07](phase1/07_LICENSE_AND_METADATA.md#catalog-sidecar-schema).

### Phase 5 — synopsis (two deployments; parked)

1. **A — JellyFlam3 Furnace** stays the Pi 5 playbook — [phase2/09_PI_FROM_SCRATCH.md](phase2/09_PI_FROM_SCRATCH.md). Not re-hosted on Ventuno.
2. **B — LLM Agent Platform** on Arduino Ventuno Q (BOM, Ubuntu, models only) — [phase5/01_VENTUNO_Q_HOST.md](phase5/01_VENTUNO_Q_HOST.md).
3. B advises A over LAN (naming / pedigree / agent CLIs); flam3 remains on A — [phase5/02_LLM_INTEGRATION.md](phase5/02_LLM_INTEGRATION.md).
4. Platform gaps on **B** — [phase5/03_AI_PLATFORM_GAPS.md](phase5/03_AI_PLATFORM_GAPS.md).
5. Do **not** implement until Owner opens the slice; do **not** overlap A and B on one board. Phase 4 products stay on Phase 4.
