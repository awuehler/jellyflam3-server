...
VoD **1.0.37+** persists each valid keyboard edit when **OK** is pressed; Back does not discard accepted edits, and Done reloads the list. Boolean/title reads are normalized at startup. Home status shows effective commercial filtering and alias fallback counts. NC tags override safe tags regardless of order, including deep links. PlayerScreen—not Video—owns playback focus so vote keys are not swallowed. **1.0.38** overlay: **OK** love, **Right** like, **Down** dismiss, **Up/Back** exit (keyboard Enter / Right / Down / Up-Esc).
