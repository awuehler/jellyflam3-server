...
1. **Posters:** filesystem under `stills/{stem}/` **and** Jellyfin Images API with retry (base64 POST; `stills/.ignore` so the scan does not attach local Primary); mid-loop frame; backfill existing flock (relocates leftover sibling `*-poster.jpg`).
