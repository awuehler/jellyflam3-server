...
systemctl is-active jellyflam3-display-sink   # active, not activating/auto-restart
...
- A wrap is one random permutation of eligible sheep (each once). VoD **1.0.31** then re-fetches Jellyfin (Limit 5000, randomly prune to 313) and starts a new mix whose first item is not the clip that just finished.
