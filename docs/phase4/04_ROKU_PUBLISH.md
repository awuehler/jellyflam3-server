...
5. Store static analysis (VoD **1.0.47**): `supports_input_launch=1` + `roInput` (documented casing `SetMessagePort` / `IsInput` / `GetInfo`); `rsg_version=1.3`; no deprecated VoD `subtitle`; `roAppMemoryMonitor` + `EnableLowGeneralMemoryEvent`; `AppDialogInitiate`/`AppDialogComplete` around pre-home Settings; `StandardKeyboardDialog` for text fields. Keep sideload zips under **4 MB** (numbered source-art PNGs stay out of the VoD package).
