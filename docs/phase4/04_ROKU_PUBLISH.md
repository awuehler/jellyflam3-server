...
5. Store static analysis (VoD **1.0.51**): HD-only `splash_screen_hd` / `mm_icon_focus_hd` (no `*_sd` keys); `supports_input_launch=1` + retained `m.input` / `roInputEvent` with `DoesExist("mediatype")` and `DoesExist("contentid")` (cert **5.2**); `rsg_version=1.3`; no deprecated VoD `subtitle`; `roAppMemoryMonitor` + `EnableLowGeneralMemoryEvent`; `AppDialogInitiate`/`AppDialogComplete` around pre-home Settings; `StandardKeyboardDialog` for text fields. Keep sideload zips under **4 MB** (numbered source-art PNGs stay out of the VoD package).
6. **Re-package before re-submitting.** Static analysis runs against the uploaded `.pkg`, so a dashboard re-scan of an old package repeats findings already fixed in-repo. Order: sideload the new zip → Installer → **Package** on that Roku → upload the new `.pkg` → re-run analysis.
7. **App Behavior Analysis runs the app, not the source.** Verify the beacons on the debug console (port **8085**) before re-submitting: launch the sideload and confirm one `AppLaunchComplete` with a plausible duration, plus paired `AppDialogInitiate`/`AppDialogComplete` if Settings opens first. A beacon signaled at `screen.Show()` measures an empty Scene — VoD **1.0.51** fires it at an operable screen instead. See [Measuring app performance](https://developer.roku.com/dev/docs/measuring-channel-performance).
8. **Cert playback runs over the public internet, not the LAN.** Testers deep link from Roku's lab, so the submitted package must carry a public `baseUrl`. Pack with `JELLYFIN_PUBLIC_URL=https://<funnel-host>` exported (otherwise `client_pack_presets.py` bakes the furnace LAN IP) and set Jellyfin's published server URL to match. Submit a low-bitrate item as the `shortFormVideo` deep-link sample: the 1080p catalog runs ~4 Mbps in ~40 s clips, and cert **3.6** wants first frame within 8 s. VoD **1.0.50** no longer escalates a failed MP4 to HLS, because Jellyfin returns these keyframe-sparse loops as one whole-clip segment.
9. **Screensaver package is not yet cert-clean.** `roku-screensaver/` still has `rsg_version=1.2`, a deprecated `subtitle`, `KeyboardDialog`, no memory-monitor APIs, and no beacons. Fix those before submitting **JellyFlam3 Dreams**.
